...
data = np.array([toolbox.extaggregate(toolbox.chaggregate(file['preictal_segment_' + str(1)][0][0]['data']))])
for i in range(1, preictals):
    ...
    data = np.append(data, np.array(
        [toolbox.extaggregate(toolbox.chaggregate(file['preictal_segment_' + str(i + 1)][0][0]['data']))]), axis=0)
...
for i in range(interictals):  # 100):#480):
    ...
    data = np.append(data, np.array(
        [toolbox.extaggregate(toolbox.chaggregate(file['interictal_segment_' + str(i + 1)][0][0]['data']))]), axis=0)
